...
def render_multilingual_rag_page(active_api_key, selected_model, active_groq_key):
    ...
    <div class="header-container" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 10px;">
        <div>
            ...
            <p class="subtitle" style="font-size: 0.8rem; margin: 0;">Query documents or translate text directly in multiple languages.</p>
        </div>
    </div>
    ...
    tab_rag, tab_direct = st.tabs(["Document RAG Translator", "Direct Text Translator"])
    ...
    with tab_rag:
        ...
        if not st.session_state.processed:
            ...
            query = st.chat_input(f"Ask a question in English or {st.session_state.target_language}...", key="multilingual_chat_input")
            if query:
